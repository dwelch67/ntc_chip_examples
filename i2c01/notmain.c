...
//xdel=0x100000;
    ...
    for(ra=0x00;ra<0x7F;ra++)
    {
        //hexstrings(ra);
        i2c_start();
        for(rb=0x40;rb;rb>>=1)
        {
            if(rb&ra) i2c_one(); else i2c_zero();
        }
        i2c_zero(); //write bit
//        i2c_one(); //read bit
        if(i2c_read())
    ...
    addr=0x34;
    for(ra=0x80;ra<0x90;ra++)
    {
        i2c_read_byte(addr,ra);
    }
        ra=i2c_read_byte(addr,0x32); hexstring(ra);
        i2c_read_byte(addr,0x36);
        i2c_write_byte(addr,0x36,0x9C); 


        ra=i2c_read_byte(addr,0x32);
        //ra|=0x80; //POWER OFF!!!
        i2c_write_byte(addr,0x32,ra);

        i2c_read_byte(addr,0x93);
